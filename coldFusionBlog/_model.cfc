...
<cffunction name="get_all_categories" returntype="Query" hint="I return all the category names and id's along with the count for each category">
	...
	<cftry>
		...
		<cfquery datasource="#variables.datasource#" name="local.all_categories" timeout="3">
			SELECT 
				category_name,
				category_id,
				count(p.post_category_id) AS category_count
				
			FROM rensel_categories AS c
				LEFT JOIN rensel_posts AS p ON(c.category_id = p.post_category_id)
	...
	<cftry>
		...
		<cfquery datasource="#variables.datasource#" name="local.all_tags">
			...
			FROM rensel_tags AS t
				LEFT JOIN rensel_posts_to_tags as p ON(t.tag_id = p.tag_id)
...
<cffunction name="get_all_posts" returntype="Struct"  hint=" returns all posts with optional arguments for sorting by tag id or category id, returns the total count of posts available in assoc array with the query {posts,count} " >
	...
	<cftry>
		<cfquery datasource="#variables.datasource#" name="local.all_posts">
			SELECT post_id,
				   ...
				   u.users_name,
				   c.category_name 
			FROM rensel_posts AS p
				INNER JOIN rensel_users AS u ON(u.users_id = p.post_user_id)
				INNER JOIN rensel_categories AS c ON(c.category_id = p.post_category_id)
	...
	<cfif url.sort_by_category neq "">
		<cftry>
			...
			FROM rensel_posts AS p
				INNER JOIN rensel_users AS u ON(u.users_id = p.post_user_id)
				INNER JOIN rensel_categories AS c ON(c.category_id = p.post_category_id) AND (c.category_id = <cfqueryparam value="#url.sort_by_category#" cfsqltype="cf_sql_integer">)
			ORDER BY post_date_posted DESC
			LIMIT #(url.page-1) * posts_per_page#,#posts_per_page#
			</cfquery>
		...
		<cftry>
			<cfquery datasource="#variables.datasource#" name="local.tag_posts">
				...
				FROM rensel_posts AS p
					INNER JOIN rensel_users AS u ON(u.users_id = p.post_user_id)
					INNER JOIN rensel_categories AS c ON(c.category_id = p.post_category_id)
				WHERE post_id IN(SELECT post_id FROM rensel_posts_to_tags WHERE tag_id=<cfqueryparam value="#url.sort_by_tag#" cfsqltype="cf_sql_integer" >)
	...
	<cfargument name="session" required="true" type="struct" >
		...
		<cfif form.add_tag neq "" and form.button eq "Add">
			<cfquery datasource="#variables.datasource#">
				INSERT INTO rensel_tags SET tag_name = <cfqueryparam value="#form.add_tag#" cfsqltype="cf_sql_varchar"  >
			</cfquery>
		...
		<cfif form.edit_tag neq "" and form.button eq "Edit">
			<cfquery datasource="#variables.datasource#">
				UPDATE rensel_tags SET tag_name=<cfqueryparam value="#form.edit_tag#" cfsqltype="cf_sql_varchar"  > WHERE tag_id='#form.edit_tag_id#'
			</cfquery>
		...
		<cfif url.delete_tag neq "">
			<cfquery datasource="#variables.datasource#">
				DELETE FROM rensel_tags WHERE tag_id='#url.delete_tag#'
			</cfquery>
			
			<cfquery datasource="#variables.datasource#">
				DELETE FROM rensel_posts_to_tags WHERE tag_id='#url.delete_tag#'
			</cfquery>
		...
		<cfif form.edit_category neq "" and form.button eq "Edit">
			<cfquery datasource="#variables.datasource#">
				UPDATE rensel_categories SET category_name=<cfqueryparam value="#form.edit_category#" cfsqltype="cf_sql_varchar"  > WHERE category_id='#form.edit_category_id#'
			</cfquery>
		...
		<cfif url.delete_category neq "">
			<cfquery datasource="#variables.datasource#">
				DELETE FROM rensel_categories WHERE category_id='#url.delete_category#'
			</cfquery>
		...
		<cfif form.add_category neq "" and form.button eq "Add">
			<cfquery datasource="#variables.datasource#">
				INSERT INTO rensel_categories SET category_name = <cfqueryparam value="#form.add_category#" cfsqltype="cf_sql_varchar"  >
			</cfquery>
...
<cffunction name="modify_posts" returntype="void" hint="modify the post db entries">
	...
	<cfif form.button eq "Add Post">
		<cfquery datasource="#variables.datasource#">
			INSERT INTO rensel_posts SET post_title='#form.title#', post_content='#form.form_content#', post_date_posted=NOW(), post_user_id='#user_id#',post_category_id='#form.category#'
		</cfquery>
		<cfquery datasource="#variables.datasource#" name="last_insert_id">
		...
		<cfloop array="#tags#" index="tag">
			<cfquery datasource="#variables.datasource#">
				INSERT INTO rensel_posts_to_tags SET post_id='#last_id#',tag_id='#tag#'
			</cfquery>
		</cfloop>
	</cfif>
	...
	<cfif form.button eq "Edit Post">
		...
		<cfquery datasource="#variables.datasource#">
			DELETE FROM rensel_posts_to_tags
			WHERE post_id='#form.id#'
		</cfquery>
		
		<cfloop array="#tags#" index="tag">
			<cfquery datasource="#variables.datasource#">
				INSERT INTO rensel_posts_to_tags SET post_id='#form.id#',tag_id='#tag#'
			</cfquery>
		</cfloop>
	</cfif>
	...
	<cfif url.delete_post neq "">
		<cfquery datasource="#variables.datasource#">
			DELETE FROM rensel_posts WHERE post_id='#url.delete_post#'
		</cfquery>
		
		<cfquery datasource="#variables.datasource#">
			DELETE FROM rensel_posts_to_tags WHERE post_id='#url.delete_post#'
		</cfquery>
	</cfif>
...
<cffunction name="get_login_info" returntype="Query" hint="returns login information query">
	...
	<cfquery datasource="#variables.datasource#" name="login">
	SELECT users_id,users_name FROM rensel_users WHERE users_name = <cfqueryparam value="#form.username#" cfsqltype="cf_sql_varchar" > AND password = <cfqueryparam value="#form.password#" cfsqltype="cf_sql_varchar" >
	</cfquery>
